...
if __name__ == "__main__":
    ...
    parser.add_argument("--out_path", type=str, help="path to the out file generated by the test", default="teb_out.txt")
    ...
    for idx in range(300):
        path_file_name = join(base_path, "worlds/BARN/path_files", "path_%d.npy" %(idx))
        ...
        optimal_times[idx] = path_length / 2
    ...
    for idx in range(300):
        if not idx in results.keys():
            print("Missing world_%d" %(idx))
        elif len(results[idx]) < 10:
            print("Test on world_%d not finished (%d/10)" %(idx, len(results[idx])))
